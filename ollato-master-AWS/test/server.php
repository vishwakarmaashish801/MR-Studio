...
if($payment_status == 'Active'){
            ...
            $query = "SELECT * FROM questions ORDER BY sr_no ASC LIMIT $start_from, $record_per_page";  
             ...
             {  
                  $output .= '  
                       <tr class="td-data">  
                            <td class="td_sr">'.$row["sr_no"].'</td>  
                            <td class="td_question">'.$row["question"].'</td>
            
                            <td class="option-col td_option">'."<label><input type='radio' class='question-option'  name='choice-".$row["sr_no"]."' domain='".$row["domain"]."' domainno='domain-".$row["sr_no"]."' value='".$row["option1"]."'> ".$row["option1"].'</label> </td>
                            <td class="option-col td_option">'."<label><input type='radio' class='question-option'  name='choice-".$row["sr_no"]."' domain='".$row["domain"]."' domainno='domain-".$row["sr_no"]."' value='".$row["option2"]."'> ".$row["option2"].'</label> </td>
             ...
             echo $output;  
              ...
              $payment_fail  .='<div class="col-md-6 mx-auto">
                                ...
                                <div class="card-body">
                                 ...
                                 <a href="/test/package.php" class="btn btn-primary">Back to Package OR Payment Mode</a> 
                                 </div></div>';
               echo $payment_fail;
           }
