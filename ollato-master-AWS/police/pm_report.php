...
// echo "User ID ". $user_id;
 ...
 $retval = mysqli_query($link, $sql1);
  ...
  while ($row = mysqli_fetch_array($retval)) {
    ...
    $Domain_name = array('Anxiety','Depression', 'Stress', 'PTSD', 'Burnout', 'Job Security', 'Work Environment', 'Work Load', 'Job Satisfaction', 'Work Life Balance','Career Opportunities','Coping Mechanism','Family Support');
    
    array_push($Domain_value_db,"$anx", "$dep", "$ss", "$pstd", "$burn", "$js", "$we", "$wl", "$jobs","$wlb","$co","$cm","$fs");
...
<body>
        ...
        <!--button-->
          <div class="container gap-2 text-center" style="margin-right:-180px">
              ...
              <div><a href="pm_dashboard.php"><button type="button"  style="background:#56cce1;color:#11486d" class="rounded-pill btn-home"> Home</button></a></div>
          </div>
                            ...
                            if($key == "Family Support") {
                                ...
                                if($value >= 10 && $value <= 12) {
                                    ...
                                    for($fc;$fc<=$total;$fc++)
                                    {
                                        ...
                                        parent.document.getElementById("content_fem").innerHTML="<div>'.$content_fem.'</div>";
                                        </script>';
