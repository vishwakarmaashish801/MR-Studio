...
<style>
    ...
    <body>
        ...
        <?php
            ...
            $query = "SELECT * FROM questions_male";
                                    ...
                                    <h3 class=" my-2 fw-bold text-center"> Work Professional Assestment</h3>
                                      ...
                                      <p class=" my-2 fw-bold text-center">login Again</p>
                                       <div class="container align-middle">
                                        <p class="fw-bold text-center"><span class="align-middle">Click Here </span>&nbsp; <a href="pm_dashboard.php"  class="btn border align-middle btn-outline-primary"> Home</a></p>
                                        </div>
                                       </div>
     ...
     <?php
       ...
       if (isset($_SESSION['$domainarray'])) {
        ...
        if (!empty($copy["Work Environment"])) {
            $we =$copy["Work Environment"];
        } else {
            $we = 0;
        }

        if (!empty($copy["Work Load"])) {
            $wl = $copy["Work Load"];
        } else {
            $wl = 0;
        }

        if (!empty($copy["Job Satisfaction"])) {
            $js = $copy["Job Satisfaction"];
        } else {
            $js = 0;
        }

    //   Add New Daomin Fro array to database

        if (!empty($copy["Work Life Balance"])) {
            $wlb = $copy["Work Life Balance"];
        } else {
            $wlb = 0;
        }
        
         if (!empty($copy["Career Opportunities"])) {
            $co = $copy["Career Opportunities"];
        } else {
            $co = 0;
        }
        
         if (!empty($copy["Coping Mechanism"])) {
            $cm = $copy["Coping Mechanism"];
        } else {
            $cm = 0;
        }
        
         if (!empty($copy["Family Support"])) {
            $fs = $copy["Family Support"];
        } else {
            $fs = 0;
        }
        
        if (!empty($copy["Female Police Person"])) {
            $fp = $copy["Female Police Person"];
        } else {
            $fp = 0;
        }
        ...
        if (mysqli_num_rows($check_studentid) > 0 ) {
            //  $status = 'Approved';
             ...
             $update = "UPDATE results SET user_name = '$st_name', Anxiety = '$anx', Depression = '$dep', Stress = '$ss', PTSD = '$pstd', Burnout = '$burn', Job_Security = '$jobs', Work_Environment = '$we', Work_Load = '$wl', Job_Satisfaction = '$js', Work_Life_Balance = '$wlb',Career_Opportunities = '$co', Coping_Mechanism = '$cm',Family_Support = '$fs' WHERE user_id = $user_id; ";
                                   ...
                                   <div class="container my-5">
                                     ...
                                     <div class="my-2 container d-flex justify-content-center gap-5">
                                      ...
                                      <p class=" my-2 fw-bold text-center">Error Server Timeout ?? Please Try Some Time</p>
                                       <div class="container align-middle">
                                        <!-- <p class="fw-bold text-center"><span class="align-middle">Click Here </span>&nbsp;<a href="pm_dashboard.php"  class="btn border align-middle btn-outline-primary">Home</a></p> -->
                                        <p class="fw-bold text-center"><span class="align-middle">Click Here </span>&nbsp; <a href="pm_dashboard.php"  class="btn border align-middle btn-outline-primary">Dashboard</a>&nbsp;&nbsp; <a href="pm_report.php" class="btn border align-middle btn-outline-primary">View Result</a></p>
            ...
            //  $status= 'Pending'; 
             ...
             $insert = "INSERT IGNORE INTO result (sr_no,user_id,user_name,Anxiety,Depression,Stress,PTSD,Burnout,Job_Security,Work_Environment,Work_Load,Job_Satisfaction,Work_Life_Balance,Career_Opportunities,Coping_Mechanism,Family_Support,Female_Domain,exam_date
                                         ) VALUES ('','$user_id','$st_name','$anx','$dep','$ss','$pstd','$burn','$jobs','$we','$wl','$js','$wlb','$co','$cm','$fs','$fp',now())";
                                   ...
                                   <div class="my-2 container d-flex justify-content-center gap-5">
                                     ...
                                     <p class=" my-2 fw-bold text-center">Unsuccessfully</p>
                                      <p class=" my-2 fw-bold text-center">Error Insertion data Server Timeout ?? Please Try Some Time</p>
                                       <div class="container align-middle">
                                        <!-- <p class="fw-bold text-center"><span class="align-middle">Click Here </span>&nbsp; <a href="pm_dashboard.php"  class="btn border align-middle btn-outline-primary">Cancel</a></p> -->
                                        <p class="fw-bold text-center"><span class="align-middle">Click Here </span>&nbsp; <a href="pm_dashboard.php"  class="btn border align-middle btn-outline-primary">Dashboard</a>&nbsp;&nbsp; <a href="pm_report.php" class="btn border align-middle btn-outline-primary">View Result</a></p>
